...
namespace Klient_1
{
    internal class Program
    {
        public static Task Handle(ConsumeContext<Wiadomosci.PytanieoPotwierdzenie> ctx)
        {
            if (ctx.Message.login == "Klient 1")
            {
                ...
                if (Console.ReadKey(true).Key == ConsoleKey.T)
                {
                    ...
                    //return Console.WriteLine("Potwierdzono zamowienie");
                }
                else
                {
                    ctx.RespondAsync(
                    ...
                    //return Console.WriteLine("Odrzucono zamowienie");
                }
            }
        ...
        static async Task Main(string[] args)
        {
            ...
            bus.Start();
            Console.WriteLine("Klient 1 wystartowal");
            ...
            while (true)
            {
                ...
                if (key == ConsoleKey.I)
                {
                    ...
                    bus.Publish(
                        new Wiadomosci.StartZamowienia() { ilosc = inputInt, login = "Klient 1" }
                    );
